...
import { petQueryKey } from "../Admin/Pet/api/pet-query-key";
import { PetStatusEnum } from "../Admin/Pet/api/schema";
...
export const Register = () => {
  ...
  const { mutateAsync: actionRegisterPlayerCustom } = useRegisterMutation();

  const onSubmit: SubmitHandler<any> = (data: RegisterPayload) => {
    data.id = Math.floor(Math.random() * 1000000);
    // actionRegisterPlayer(data);
    queryClient.invalidateQueries({
      queryKey: petQueryKey.list({
        status: PetStatusEnum.available,
      })
    });
    actionRegisterPlayerCustom(data).then(() => {
      showNotification({
        duration: 3000,
        text: "Player registered successfully, please login",
        icon: <CheckCircleOutlined />,
